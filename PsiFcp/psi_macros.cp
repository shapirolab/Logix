...
/* Manage output for psifcp to fcp transformation. */

report_psifcp(PN, FN, FGR, S, Results) :-

    FGR =?= true :
      PN = _ |
	parse#string(S, TI, PSE),
	report_psifcp_parsed;
...
report_psifcp_parsed(FN, PSE, TI, Results) :-

    PSE = [] |
	psifcp#transform([],TI,EX,TO,PTE),
	report_psifcp_to_compound;

    otherwise :
      TI = _,
      Results = [FN - "parsing errors:" | PSE].

report_psifcp_to_compound(FN, PTE, EX, TO, Results) :-

    PTE = [] |
        transform#languages([], dg, [language(compound)],_AT,TO,IO,TLE,[]),
	report_psifcp_transformed;

    otherwise :
      EX = _,
      TO = _,
      Results = [FN - "psifcp transformation errors:" | PTE].


report_psifcp_transformed(FN, TLE, EX, IO, Results) :-

    TLE = [] |
	handle_exports,
	widgets#pretty#module(EXIO,SP),
	file#put_file(FN, SP, put, FPR),
	report_psifcp_written;
...
report_psifcp_written(FN, FPR, Results) :-
