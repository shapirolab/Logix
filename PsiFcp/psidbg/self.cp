...
Currently locked by     $Locker:  $
			$Revision: 1.1 $
			$Source: /home/qiana/Repository/PsiFcp/psidbg/self.cp,v $

Copyright (C) 1988, Weizmann Institute of Science - Rehovot, ISRAEL

*/

-export([start/1,start/2,interpret/2,interpret/3,adb/3]).
-language(compound).
-mode(trust).
...
NewOptions      ::= [prefix(String) | PsiOptions].
...
interpret(Id, RPCGoal) + (PsiOptions = []) :- 
    true:
      make_channel(BadOption, _) |
	psi_utils#parse_options(PsiOptions, Depth(1), BadOption(BadOption),
		      Sender(no_sender), Which(none)),
	computation # events(Events),
	do_debug(Events, Id, RPCGoal, [Depth,Sender,Which]).

/*************************************************************************/
procedure do_debug(NonEmpty_Events, Id, RPCGoal, PsiOptions).
do_debug(NonEmpty_Events, Id, RPCGoal, PsiOptions) :-
	NonEmpty_Events?resumed :
	    NonEmpty_Events' = _ |
		adb(Id, RPCGoal,_,PsiOptions);

	NonEmpty_Events?aborted :
	    NonEmpty_Events' = _, PsiOptions = _, Id = _, RPCGoal = _ ;
...
/*************************************************************************/
procedure adb(Id,RPCGoal,Trace,PsiOptions).
adb(Id,RPCGoal,Trace) + (PsiOptions = []) :-
	...
	freeze(RPCGoal,[],FG,_) :
		...
		,psi_monitor#options(PsiOptions, OldPsiOptions)

		,computation # shell(prompt,'@debug ->')
		,computation # shell(filter_system_macros,Input,NonCommands)
		,filter(Id', Input, IO?, Events, OldPsiOptions,
			{Commands1,_Commands2,NonCommands})

		,reduce#debug(RPCGoal,Breaks,Commands1?,
				Trace,IO_Channel,Events)
...
/********************************************************************/
procedure filter(Id, In, IO, Events, OldPsiOptions, OutStreams).
filter(Id, In, IO, Events, OldPsiOptions, OutStreams) :-
	Events?Event |
		print_event(Id, Event),
		filter;

	IO =[] |
		shutdown(Id, In, IO, Events, OldPsiOptions, OutStreams);

	In ? shutdown(debug),
	OutStreams = {_Commands, _Requests, System} :
	    Id = _, IO = _, Events = _, OldPsiOptions = _,
	    System = In' |
	...
	IO=[_I|_IO'],
	unknown(Events) |
		process_io(Id, In, IO, Events, OldPsiOptions, OutStreams);
	...
	OutStreams = {Commands, Requests, System} :
	    Id = _, OldPsiOptions = _,
	    Commands=[], Requests=[], System = []  | true.



/*************************************************************************/
procedure shutdown(Id, In, CloseIOs, Events, OldPsiOptions, ChokeStreams).
shutdown(Id, In, CloseIOs, Events, OldPsiOptions, ChokeStreams) :-
	ChokeStreams = {Commands, Requests, System} :
	    Commands=[], Requests=[], System = In |
		computation # shell(pop_prompt),
		complete_io(Id, CloseIOs),
		print_all_events(Id, Events),
		psi_monitor#options(OldPsiOptions, _).
...
options(O)	Reset psifcp options.
...
",				[close([], Non_Help)]);
	...
	Command = help(options) |
		computation # display(term, "
options(O) may be used to reset the output format for psifcp goals.
...
command(I, O, Id, Commands, Commands') :-
	...
	I = spr |
		command(resolvent, O, Id, Commands, Commands');

	I = resolvent(R)	: 
	    Id = _,
	    Commands ! resolvent(R'), O = [] | 
		unify_without_failure(R',R);

	I = spr(_No, _Options) :
	    Commands = Commands', O = [] |
		computation # display((Id: not_allowed-I));

        /* Resolvent for other process? - maybe not!? */
/*
	I = resolvent(_No, _R)	:
	    Id = _,
	    O = [computation(N,N) | O'] |
		screen#display(N - which_resolvent(N?, I, O', Commands, Commands'), known(N)),
...
which_resolvent(N, I, O, Commands, Commands') :-
	I = _(No, R) :
	    No = N |
		command(resolvent(R), O, [], Commands, Commands');
...
/*************************************************************************/
procedure process_io(Id, In, NonEmpty_IO, Events, OldPsiOptions, OutStreams).
process_io(Id, In, NonEmpty_IO, Events, OldPsiOptions, OutStreams) :-
	NonEmpty_IO?display(List, How), List = Goal\End :
	  End = ['
']	    |    % put a carriage return at the end
		computation # display(term, [Id, ' ' | Goal], How),
		filter(Id, In, NonEmpty_IO', Events,
			OldPsiOptions, OutStreams);
	...
	Ask = ask(_Prompt,_Response,_How,_Print_Requests) |
		read(Id, In, NonEmpty_IO', Events,
			OldPsiOptions, OutStreams, Ask).
...
/*************************************************************************/
procedure read(Id, In, IO, Events, OldPsiOptions, OutStreams, Ask).
read(Id, In, IO, Events, OldPsiOptions, OutStreams, Ask) :-
...
%computation # display(term, response(Answer,Ok),[known(Answer), known(Ok)]),
		check_parse(Ok, Id, In, IO, Events, OldPsiOptions,
				OutStreams, Ask, Answer).

/*************************************************************************/
/*************************************************************************/
procedure check_parse(Ok2, Id, In, IO, Events, OldPsiOptions,
			OutStreams, Ask, Answer).
check_parse(Ok2, Id, In, IO, Events, OldPsiOptions, OutStreams, Ask, Answer) :-
	Ok2 = [] |
%computation # display(term, check_parse(ok,Answer)),
		wait(Id, In, IO, Events, OldPsiOptions,
			OutStreams, Ask, Answer);
	...
	Ok2 =\= [] :
		...
		call(Done,read(Id, In, IO, Events, OldPsiOptions,
				OutStreams, Ask)).
...
call(Done, Call) :-
	known(Done),
	Call =  read(Id, In, IO, Events, OldPsiOptions, OutStreams, Ask) |
		read(Id, In, IO, Events, OldPsiOptions, OutStreams, Ask);

	known(Done),
	Call =  print_response(Id, In, IO, Events, OldPsiOptions,
					OutStreams, Ask) |
		print_response(Id, In, IO, Events, OldPsiOptions,
					OutStreams, Ask).

/*************************************************************************/
procedure wait(Id, In, IO, Events, OldPsiOptions, OutStreams, Ask, Answer).
wait(Id, In, IO, Events, OldPsiOptions, OutStreams, Ask, Answer) :-
	Answer = [debug] |
		computation # shell(prompt,'@debug? ->'),
		temporary_command_mode(Id, In, IO, Events, OldPsiOptions,
					OutStreams, Ask);
...
/*************************************************************************/
procedure print_response(Id, In, IO, Events, OldPsiOptions, OutStreams, Ask).
print_response(Id, In, IO, Events, OldPsiOptions, OutStreams, Ask) :-
	...
	Print_Requests?display_stream(What, How) :
		...
		computation # display(stream, What, NewHow),
		call(Done,print_response(Id,In,IO,Events, OldPsiOptions,
						OutStreams,Ask'));
...
/*************************************************************************/
procedure temporary_command_mode(Id, In, IO, Events, OldPsiOptions,
					OutStreams, Ask).
temporary_command_mode(Id, In, IO, Events, OldPsiOptions, OutStreams, Ask) :-
	Events?Event |
		print_event(Id, Event),
