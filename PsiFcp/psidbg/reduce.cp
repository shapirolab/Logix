...
Last update by          $Author: bill $
			$Date: 2000/07/09 06:33:32 $
Currently locked by     $Locker:  $
			$Revision: 1.1 $
			$Source: /home/qiana/Repository/PsiFcp/psidbg/reduce.cp,v $
...
debug(RPCGoal, Breaks, Commands, Trace, IO, Events) :-
	...
	make_channel(Psi, Psi_Input),

	report_state(Commands, Circuit, Channels, Events),
	monitor(Commands,Signals,Psi_Input,Circuit,Channels),
...
/*************************************************************************/
procedure monitor(Commands,Signals,Psi_Input,Circuit,Channels).
...
monitor(Commands,Signals,Psi_Input,Circuit,Channels) :-
	...
	Commands?resolvent(X) :
	    Signals!suspend,
	    L = [resolvent(X') | L'] |
		psi_monitor#options(PsiOptions, PsiOptions),
		psi_utils#show_resolvent(X'?, PsiOptions, X),
		close_tail(Commands', Signals', Psi_Input, L'-R, Channels);
	...
	Psi_Input ? " goal "(Goal, PsiGoal) |
		psi_monitor#options(PsiOptions, PsiOptions),
		psi_utils#show_goals(Goal, PsiOptions?, PsiGoal),
		monitor;
	...
	Commands = [Command | MoreCommands] :
		...
		send_signal(Ok, Commands', Signals, Psi_Input,
				Circuit,Channels, Error).


/*************************************************************************/
procedure close_tail(Commands,Signals,Psi_Input,
			Choke_Msg_On_Right,Channels).
close_tail(Commands,Signals,Psi_Input,Choke_Msg_On_Right,
		Channels) :-
	Choke_Msg_On_Right = L-R,
	R?{_,Goals} :
	    Goals = [] |
		monitor(Commands, Signals, Psi_Input, L-R',
			Channels).
...
/*************************************************************************/
procedure send_signal(Ok,Non_Empty_Commands,Signals,Psi_Input, 
			Circuit,Channels,Error).
send_signal(Ok,Non_Empty_Commands,Signals,Psi_Input,
			Circuit,Channels,Error) :-
	Ok =\= true,
	Channels = {IO, _User, _Psi},
	Non_Empty_Commands?_Command |
		write_channel(Error, IO),
%computation # display(term, debug#reduce(unknown_request(Command))),
		monitor(Non_Empty_Commands', Signals, Psi_Input,
				Circuit, Channels);
