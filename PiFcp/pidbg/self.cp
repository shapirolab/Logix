...
/*************************************************************************/
procedure do_debug(NonEmpty_Events, Id, RPCGoal, PiOptions).
do_debug(NonEmpty_Events, Id, RPCGoal, PiOptions) :-
	NonEmpty_Events?resumed :
	    NonEmpty_Events' = _ |
		adb(Id, RPCGoal,_,PiOptions);

	NonEmpty_Events?aborted :
	    NonEmpty_Events' = _, PiOptions = _, Id = _, RPCGoal = _ ;
...
adb(Id,RPCGoal,Trace) + (PiOptions = []) :-
	...
	freeze(RPCGoal,[],FG,_) :
		...
		,reduce#debug(RPCGoal,PiOptions,Breaks,Commands1?,
