/*
Translate Pi Calculus notation to FCP.

Bill Silverman, May 2000.

Last update by		$Author: bill $
		       	$Date: 2000/05/07 09:04:26 $
Currently locked by 	$Locker:  $
			$Revision: 2.1 $
			$Source: /home/qiana/Repository/PiFcp/pifcp/pctopifcp.cp,v $
...
** Translate/4
**
** Translate Pi Calculus module to pifcp.
**
** Input:
**
**   Source      - Pi Calculus code, minus attributes.
**
** Output:
**
**   Pifcp       - Pifcp code.
...
translate(Source, Pifcp, Errors, NextErrors) :-

    true :
      Pifcp = Terms? |
	filter_pifcp_attributes(Source, Source', Terms, Terms'?),
	processes.

/* Copy Stochastic Pi Calculus Attributes. */

filter_pifcp_attributes(Source, NextSource, Terms, NextTerms) :-
...
process_rhs(PC, Pifcp, Errors, NextErrors) :-
    ...
    PC =?= (_, _) |
	compound_guard(PC, Pifcp'?, PC', Pifcp, Errors, Errors'),
	self;
    ...
    Head =?= (_, _) :
      PC'' = (Goal? | PC') |
	compound_guard(Head, Pifcp'?, Goal, Pifcp, Errors, Errors'),
	self;

    PC =?= [_ | _] |
	new_scope(PC, Pifcp, Errors, NextErrors);
