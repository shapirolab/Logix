/*
Main control of Stochastic Pi Calulus algorithmic debugger.
...
Currently locked by     $Locker:  $
			$Revision: 1.1 $
			$Source: /home/qiana/Repository/SpiFcp/spidbg/self.cp,v $
...
NewOptions      ::= [prefix(String) | SpiOptions].
...
do_debug(NonEmpty_Events, Id, RPCGoal, SpiOptions) :-
	NonEmpty_Events?resumed :
	    NonEmpty_Events' = _ |
		adb(Id, RPCGoal,_,SpiOptions);

	NonEmpty_Events?aborted :
	    NonEmpty_Events' = _, SpiOptions = _, Id = _, RPCGoal = _ ;
...
/*************************************************************************/
procedure adb(Id,RPCGoal,Trace,SpiOptions).
adb(Id,RPCGoal,Trace) + (SpiOptions = []) :-
...
/********************************************************************/
procedure filter(Id, In, IO, Events, OldSpiOptions, OutStreams).
filter(Id, In, IO, Events, OldSpiOptions, OutStreams) :-
	Events?Event |
		print_event(Id, Event),
		filter;

	IO =[] |
		shutdown(Id, In, IO, Events, OldSpiOptions, OutStreams);

	In ? shutdown(debug),
	OutStreams = {_Commands, _Requests, System} :
	    Id = _, IO = _, Events = _, OldSpiOptions = _,
	    System = In' |
	...
	IO=[_I|_IO'],
	unknown(Events) |
		process_io(Id, In, IO, Events, OldSpiOptions, OutStreams);
...
options(O)	Reset spifcp options.
...
",				[close([], Non_Help)]);
	...
	Command = help(options) |
		computation # display(term, "
options(O) may be used to reset the output format for spifcp goals.
...
/*************************************************************************/
procedure process_io(Id, In, NonEmpty_IO, Events, OldSpiOptions, OutStreams).
process_io(Id, In, NonEmpty_IO, Events, OldSpiOptions, OutStreams) :-
	NonEmpty_IO?display(List, How), List = Goal\End :
	  End = ['
']	    |    % put a carriage return at the end
		computation # display(term, [Id, ' ' | Goal], How),
		filter(Id, In, NonEmpty_IO', Events,
			OldSpiOptions, OutStreams);
...
/*************************************************************************/
procedure read(Id, In, IO, Events, OldSpiOptions, OutStreams, Ask).
read(Id, In, IO, Events, OldSpiOptions, OutStreams, Ask) :-
...
%computation # display(term, response(Answer,Ok),[known(Answer), known(Ok)]),
		check_parse(Ok, Id, In, IO, Events, OldSpiOptions,
				OutStreams, Ask, Answer).

/*************************************************************************/
/*************************************************************************/
procedure check_parse(Ok2, Id, In, IO, Events, OldSpiOptions,
			OutStreams, Ask, Answer).
check_parse(Ok2, Id, In, IO, Events, OldSpiOptions, OutStreams, Ask, Answer) :-
	Ok2 = [] |
%computation # display(term, check_parse(ok,Answer)),
		wait(Id, In, IO, Events, OldSpiOptions,
			OutStreams, Ask, Answer);
	...
	Ok2 =\= [] :
		...
		call(Done,read(Id, In, IO, Events, OldSpiOptions,
				OutStreams, Ask)).
...
call(Done, Call) :-
	known(Done),
	Call =  read(Id, In, IO, Events, OldSpiOptions, OutStreams, Ask) |
		read(Id, In, IO, Events, OldSpiOptions, OutStreams, Ask);

	known(Done),
	Call =  print_response(Id, In, IO, Events, OldSpiOptions,
					OutStreams, Ask) |
		print_response(Id, In, IO, Events, OldSpiOptions,
					OutStreams, Ask).

/*************************************************************************/
procedure wait(Id, In, IO, Events, OldSpiOptions, OutStreams, Ask, Answer).
wait(Id, In, IO, Events, OldSpiOptions, OutStreams, Ask, Answer) :-
	Answer = [debug] |
		computation # shell(prompt,'@debug? ->'),
		temporary_command_mode(Id, In, IO, Events, OldSpiOptions,
					OutStreams, Ask);
...
/*************************************************************************/
procedure print_response(Id, In, IO, Events, OldSpiOptions, OutStreams, Ask).
print_response(Id, In, IO, Events, OldSpiOptions, OutStreams, Ask) :-
	...
	Print_Requests?display_stream(What, How) :
		...
		computation # display(stream, What, NewHow),
		call(Done,print_response(Id,In,IO,Events, OldSpiOptions,
						OutStreams,Ask'));
...
/*************************************************************************/
procedure temporary_command_mode(Id, In, IO, Events, OldSpiOptions,
					OutStreams, Ask).
temporary_command_mode(Id, In, IO, Events, OldSpiOptions, OutStreams, Ask) :-
	Events?Event |
		print_event(Id, Event),
