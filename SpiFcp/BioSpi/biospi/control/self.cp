...
Currently locked by 	$Locker:  $
			$Revision: 1.1 $
			$Source: /home/qiana/Repository/SpiFcp/BioSpi/biospi/control/Attic/self.cp,v $
...
add_short_circuit(Mode, Program, NewProgram, PIds, TIds) :-
    ...
    Program ? procedure(Ident, Clauses) :
      ...
      PIds ! lookup(CommFunctor?, Value, Value, Status) |
add_lookup(Status?) = add_lookup(new),
	add_short_circuit,
	new_ident(Ident,  Mode, NewIdent, Reply),
	factor_ident(Ident, _Function, Functor, _Alias, Arguments),
	string_to_dlist(Functor?, FL, [COMMUNICATION_SUFFIX]),
	list_to_string(FL?, CommFunctor),
