/*
Precompiler for Stochastic Pi Calculus procedures - call management.

Bill Silverman, December 1999.

Last update by		$Author: bill $
		       	$Date: 2006/08/05 05:42:44 $
...
make_local_call(ProcessDefinition, Locals, Primes, Body1, Body2,
  ...
  verify_status_list(StatusList, VerifiedStatusList) :-
	...
	utilities#subtract_list(StatusList,
		[blocked, type, baserate, references,
...
make_summed_rhs(Name, Calls, CodeTuples, Index, Prepares, Code, Results,
  ...
  add_sends_and_receives(Asks, Tells, ClauseList, Name,
    ...
    Communication =?= (`spifcp(chosen) = _Index : Result = Tuple | Body),
    Index++ :
      Prepares ! {Type(ChannelName), Identify, Request'?},
      Results ! Result,
      Code ! Index((`spifcp(chosen) = Index : Result = Tuple | Body)) |
...
make_sum_procedure(Mode, Name, Requests, RHS, Tuple, Results,
	...
	make_choice_atom(Atom, SendChoices?, [`spifcp(chosen) | Results],
				ChoiceAtom);
